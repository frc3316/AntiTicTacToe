public class GameEngine {
	...
	final int maxGames = 500;

	public void runGame() {
		/*
		 * Players to be configured here
		 */
		Player player1 = new SmartBot(PlayerType.A);
		...
		playerStarts = playerStarts == PlayerType.A
				? PlayerType.B
				: PlayerType.A;
		...
		Player currentPlayer = player1.getSymbol() == playerStarts
				? player1
				: player2;
		...
		while (!board.winner() && !board.isFull()) {
			...
			if (next.x > 2 || next.y > 2 || next.x < 0 || next.y < 0) {
				System.out.println("\t" + currentPlayer
						+ " lost because he tried playing outside of the board");
				...
				System.out.println("\t" + currentPlayer
						+ " lost because he tried playing in an already taken space");
				if (currentPlayer.getSymbol() == PlayerType.A)
					player2Score++;
				else
					player1Score++;
				...
				System.out.println("\t" + currentPlayer
						+ " lost because he tried cheating - placing a symbol that is not his");
				if (currentPlayer.getSymbol() == PlayerType.A)
					player1Score++;
				else
					player2Score++;
		...
		if (board.winner()) {
			Player winnerPlayer = currentPlayer.getSymbol() == PlayerType.A
					? player2
					: player1;
			...
			System.out
					.println("\n" + totalGames
							+ (totalGames != 1
									...
									: " game was played")
							+ ":");
			System.out.println("\t" + player1.name + " won in " + player1Score
					+ (player1Score != 1 ? " games" : " game") + " - "
					+ ((double) player1Score / totalGames * 100)
					+ "% of all the games.");
			System.out.println("\t" + player2.name + " won in " + player2Score
					+ (player2Score != 1 ? " games" : " game") + " - "
					+ ((double) player2Score / totalGames * 100)
					+ "% of all the games.");
			System.out.println("\tIt was a tie in " + scoreTie
					+ (scoreTie != 1 ? " games" : " game") + " - "
					+ ((double) scoreTie / totalGames * 100)
					+ "% of all the games.");
		}
	}
